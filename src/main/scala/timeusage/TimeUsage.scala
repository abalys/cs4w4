...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    StructType(
      ...
      columnNames.tail.map(col => StructField(col, DoubleType, false))
    )
  }
  ...
  def row(line: List[String]): Row =
    Row(line.head :: line.tail.map(x => x.toDouble): _*)
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primList = List("t01","t03","t11","t1801","t1803")
    val primaryNeeds: List[String] = columnNames.filter(col => primList.exists(col.startsWith))
    val workList = List("t05","t1805")
    val work: List[String] = columnNames.filter(col => workList.exists(col.startsWith))
    val leisureList = List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")
    val leisure = columnNames
      .filter(col => leisureList.exists(col.startsWith))
      .filter(!work.contains(_))
      .filter(!primaryNeeds.contains(_))

    (primaryNeeds.map(col), work.map(col), leisure.map(col))
  }
